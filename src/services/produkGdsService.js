...
class ProdukGdsService {
  ...
  static async update(pubid, data) {
    // Determine pid: if pubid contains encoded PID, use it. But in FE hook, it only gives pubid or id.
    // It's safer to use the encrypted pid generated by backend mapped in getData.
    // So we change the interface to receive pid instead of pubid.
    const response = await HttpClient.post(`${API_ENDPOINTS.MASTER.BARANG}/update`, { ...data, pid: pubid });
    return {
      success: true,
      data: response.data,
  ...
  static async delete(pubid) {
    const response = await HttpClient.post(`${API_ENDPOINTS.MASTER.BARANG}/hapus`, { pid: pubid });
    return {
      success: true,
      data: response.data,
