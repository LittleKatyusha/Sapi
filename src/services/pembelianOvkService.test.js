...
import service from './pembelianOvkService';

jest.mock('./httpClient', () => ({ __esModule: true, default: { get: jest.fn() } }));
beforeEach(() => { global.Blob = Blob; jest.clearAllMocks(); });

test.each(['purchase', 'stock'])('downloads %s using encrypted record PID', async type => {
  ...
  expect(httpClient.get).toHaveBeenCalledWith('/api/rph/pembelian-ovk/document', {
