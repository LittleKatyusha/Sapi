...
const LoadingSpinner = ({ size = 'medium', message = 'Memuat halaman...' }) => {
  const sizeClasses = {
    small: 'w-4 h-4',
    medium: 'w-8 h-8',
    large: 'w-12 h-12'
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[200px] p-8">
      ...
      {message && (
        <p className="mt-4 text-emerald-700 text-sm font-medium animate-pulse">
          {message}
        </p>
      )}
    </div>
  ...
  return (
    <div className="fixed inset-0 bg-white bg-opacity-90 flex items-center justify-center z-50">
      <div className="text-center">
        ...
        <p className="mt-4 text-emerald-700 font-medium">{message}</p>
      </div>
    </div>
  );
};
