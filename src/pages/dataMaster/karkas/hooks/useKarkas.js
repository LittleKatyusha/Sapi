...
const useKarkas = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await HttpClient.get(`${API_ENDPOINTS.MASTER.KARKAS}/data`, { cache: true });
  ...
  const createItem = useCallback(async (payload) => {
    ...
    setError(null);
    try {
      const result = await HttpClient.post(`${API_ENDPOINTS.MASTER.KARKAS}/store`, payload);
      if (result?.status === 'ok' || result?.data) {
        HttpClient.clearCache?.('karkas');
        await fetchData();
        return result;
      }
  ...
  const updateItem = useCallback(async (pid, payload) => {
    ...
    setError(null);
    try {
      const result = await HttpClient.post(`${API_ENDPOINTS.MASTER.KARKAS}/update`, { pid, ...payload });
      if (result?.status === 'ok' || result?.data) {
        HttpClient.clearCache?.('karkas');
        await fetchData();
        return result;
  ...
  const deleteItem = useCallback(async (pid) => {
    ...
    setError(null);
    try {
      const result = await HttpClient.post(`${API_ENDPOINTS.MASTER.KARKAS}/hapus`, { pid });
  ...
  const stats = useMemo(() => ({
    total: data.length,
    totalDetails: data.reduce((sum, item) => sum + (item.details_count || item.details?.length || 0), 0),
  }), [data]);

  return { data, loading, error, fetchData, createItem, updateItem, deleteItem, searchTerm, setSearchTerm, stats };
};

export default useKarkas;
