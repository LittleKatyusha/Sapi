...
import {
  ...
  MoreVertical, Eye, Pencil, Trash2, FileText, AlertCircle,
...
const STORAGE_KEY = 'persetujuan_feedmil_state_v1';
...
const PersetujuanFeedmilPage = () => {
  ...
  const totalRecords = persetujuanFeedmil.length;
  ...
  useEffect(() => {
    if (fetchTrigger > 0) fetchPersetujuanFeedmil();
  }, [fetchTrigger, fetchPersetujuanFeedmil]);
  ...
  const handleSave = useCallback(async (payload) => {
    try {
      if (editData) {
        await updatePersetujuanFeedmil(editData.pid || editData.pubid, payload);
        showNotif('success', 'Persetujuan Feedmil berhasil diperbarui');
      } else {
        await createPersetujuanFeedmil(payload);
        showNotif('success', 'Persetujuan Feedmil berhasil ditambahkan');
      }
      setShowModal(false);
      setEditData(null);
    } catch (err) {
  ...
  }, [editData, updatePersetujuanFeedmil, createPersetujuanFeedmil, showNotif]);
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      setDeleteData(null);
      showNotif('success', 'Persetujuan Feedmil berhasil dihapus');
    } catch (err) {
      showNotif('error', err.message || 'Gagal menghapus data');
    } finally {
      setIsDeleting(false);
    }
  }, [deleteData, deletePersetujuanFeedmil, showNotif]);
    ...
    return [
      ...
      },
      {
        name: (
          ...
          >
            <span>Nama Persetujuan Feedmil</span>
            {renderSortIcon(1)}
          </button>
        ),
        grow: 1.6,
      ...
      <header className="shrink-0 border-b border-slate-200 bg-white">
        <div className="flex items-center justify-between gap-4 px-4 sm:px-6 py-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-bold tracking-tight text-slate-900 truncate">Master Persetujuan Feedmil</h1>
              <p className="text-xs text-slate-500 truncate hidden sm:block">Kelola data master persetujuan Feedmil</p>
            </div>
          </div>
          <button
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  <input
                    ...
                    placeholder="Nama persetujuan Feedmil"
              ...
              noDataComponent={
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    {hasAppliedFilters ? 'Coba ubah filter atau reset' : 'Belum ada persetujuan Feedmil terdaftar'}
                  </p>
                </div>
              }
              highlightOnHover
    ...
    <div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right">
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Detail Persetujuan Feedmil</h3>
            <p className="text-xs text-slate-500">Informasi lengkap persetujuan Feedmil</p>
          </div>
