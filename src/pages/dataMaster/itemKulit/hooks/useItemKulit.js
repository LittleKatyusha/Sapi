...
const useItemKulit = () => {
  ...
  const API_BASE = API_ENDPOINTS.MASTER.ITEM_KULIT;

  // Fetch data server-side with params
  const fetchItemKulit = useCallback(async (params = {}) => {
    setLoading(true);
    setError(null);
    try {
  ...
  // Create item kulit
  const createItemKulit = useCallback(async (payload) => {
    setLoading(true);
    setError(null);
    try {
  ...
  // Update item kulit
  const updateItemKulit = useCallback(async (pid, payload) => {
    setLoading(true);
    setError(null);
    try {
