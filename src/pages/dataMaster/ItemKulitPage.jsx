...
const STORAGE_KEY = 'item_kulit_state_v1';

const ItemKulitPage = () => {
  const {
    loading, error, fetchItemKulit,
    createItemKulit, updateItemKulit, deleteItemKulit,
  } = useItemKulit();
  ...
  const loadData = useCallback(async () => {
    ...
    const result = await fetchItemKulit({
      start: (cp - 1) * pp,
      length: pp,
      orderColumn: sc,
      orderDir: sd,
  ...
  const handleSave = useCallback(async (payload) => {
    try {
      if (editData) {
        const result = await updateItemKulit(editData.pid, payload);
        if (result.success) {
          showNotif('success', 'Item kulit berhasil diperbarui');
          setShowModal(false);
          setEditData(null);
          setFetchTrigger((t) => t + 1);
        } else {
        ...
        const result = await createItemKulit(payload);
        if (result.success) {
          showNotif('success', 'Item kulit berhasil ditambahkan');
          setShowModal(false);
          setFetchTrigger((t) => t + 1);
        } else {
  ...
  }, [editData, updateItemKulit, createItemKulit, showNotif]);

  const handleConfirmDelete = useCallback(async () => {
    if (!deleteData) return;
    setIsDeleting(true);
    try {
      const result = await deleteItemKulit(deleteData.pid);
      if (result.success) {
        setDeleteData(null);
        showNotif('success', 'Item kulit berhasil dihapus');
        setFetchTrigger((t) => t + 1);
      } else {
  ...
  }, [deleteData, deleteItemKulit, showNotif]);
      ...
      <header className="shrink-0 border-b border-slate-200 bg-white">
        <div className="flex items-center justify-between gap-4 px-4 sm:px-6 py-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-bold tracking-tight text-slate-900 truncate">Master Item Kulit</h1>
              <p className="text-xs text-slate-500 truncate hidden sm:block">Kelola data master item kulit</p>
            </div>
          </div>
          <button
              ...
              noDataComponent={
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    {hasAppliedFilters ? 'Coba ubah filter atau reset' : 'Belum ada item kulit terdaftar'}
                  </p>
                </div>
              }
              highlightOnHover
    ...
    <div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right">
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Detail Item Kulit</h3>
            <p className="text-xs text-slate-500">Informasi lengkap item kulit</p>
          </div>
