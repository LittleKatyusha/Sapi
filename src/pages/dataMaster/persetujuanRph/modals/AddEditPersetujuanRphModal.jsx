...
const AddEditPersetujuanRphModal = ({ 
  item, 
  onClose, 
  onSave, 
  loading = false 
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = 'Nama persetujuan RPH wajib diisi';
    } else if (formData.name.length > 200) {
      newErrors.name = 'Nama persetujuan RPH maksimal 200 karakter';
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">
            {item ? 'Edit Persetujuan RPH' : 'Tambah Persetujuan RPH'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* Nama Persetujuan RPH */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Nama Persetujuan RPH <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={formData.name}
              ...
              placeholder="Masukkan nama persetujuan RPH"
              disabled={isSubmitting}
              maxLength={200}
            />
            {errors.name && (
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Masukkan deskripsi persetujuan RPH"
              disabled={isSubmitting}
            />
            {errors.description && (
              <p className="mt-1 text-sm text-red-600 flex items-center gap-1">
...
export default AddEditPersetujuanRphModal;