...
const STORAGE_KEY = 'item_feedmil_state_v1';

const ItemFeedmilPage = () => {
  const {
    loading, error, fetchItemFeedmil,
    createItemFeedmil, updateItemFeedmil, deleteItemFeedmil,
  } = useItemFeedmil();
  ...
  const loadData = useCallback(async () => {
    ...
    const result = await fetchItemFeedmil({
      start: (cp - 1) * pp,
      length: pp,
      orderColumn: sc,
      orderDir: sd,
  ...
  const handleSave = useCallback(async (payload) => {
    try {
      if (editData) {
        const result = await updateItemFeedmil(editData.pid, payload);
        if (result.success) {
          showNotif('success', 'Item feedmil berhasil diperbarui');
          setShowModal(false);
          setEditData(null);
          setFetchTrigger((t) => t + 1);
        } else {
        ...
        const result = await createItemFeedmil(payload);
        if (result.success) {
          showNotif('success', 'Item feedmil berhasil ditambahkan');
          setShowModal(false);
          setFetchTrigger((t) => t + 1);
        } else {
  ...
  }, [editData, updateItemFeedmil, createItemFeedmil, showNotif]);

  const handleConfirmDelete = useCallback(async () => {
    if (!deleteData) return;
    setIsDeleting(true);
    try {
      const result = await deleteItemFeedmil(deleteData.pid);
      if (result.success) {
        setDeleteData(null);
        showNotif('success', 'Item feedmil berhasil dihapus');
        setFetchTrigger((t) => t + 1);
      } else {
  ...
  }, [deleteData, deleteItemFeedmil, showNotif]);
      ...
      <header className="shrink-0 border-b border-slate-200 bg-white">
        <div className="flex items-center justify-between gap-4 px-4 sm:px-6 py-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-bold tracking-tight text-slate-900 truncate">Master Item Feedmil</h1>
              <p className="text-xs text-slate-500 truncate hidden sm:block">Kelola data master item feedmil</p>
            </div>
          </div>
          <button
          ...
          <div className="shrink-0 flex flex-col gap-3 border-b border-slate-100 px-4 py-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                Filter
                {hasAppliedFilters && (
                  <span className="inline-flex items-center justify-center rounded-full bg-emerald-600 px-1.5 text-[10px] font-bold text-white">
              ...
              noDataComponent={
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    {hasAppliedFilters ? 'Coba ubah filter atau reset' : 'Belum ada item feedmil terdaftar'}
                  </p>
                </div>
              }
              highlightOnHover
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-1 px-2 text-xs text-slate-600">
                ...
                <input
                  ...
                  className="w-12 rounded-md border border-slate-200 px-1.5 py-1 text-xs text-center outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-100"
                />
      ...
      {isOpen && createPortal(
        ...
        >
          ...
          <button
            ...
            className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-slate-50 flex items-center gap-2 transition-colors"
          >
            <Pencil className="h-3.5 w-3.5 text-emerald-500" /> Edit
          </button>
          <button
    ...
    <div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right">
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Detail Item Feedmil</h3>
            <p className="text-xs text-slate-500">Informasi lengkap item feedmil</p>
          </div>
        ...
        <div className="shrink-0 flex gap-2 px-4 py-3 border-t border-slate-100 bg-slate-50/50">
          ...
          <button
            onClick={() => onEdit(row)}
            className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-700 transition-colors"
          >
