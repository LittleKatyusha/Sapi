import React from 'react';
import { Ruler } from 'lucide-react';

import MasterDataTablePage from './pembeliHo/components/MasterDataTablePage';
import useSatuan from './satuan/hooks/useSatuan';
import AddEditSatuanModal from './satuan/modals/AddEditSatuanModal';
import DeleteConfirmationModal from './satuan/modals/DeleteConfirmationModal';

const SatuanPage = () => {
  const hook = useSatuan();

  const mappedHook = {
    loading: hook.loading,
    error: hook.error,
    fetch: hook.fetchSatuan,
    create: hook.createSatuan,
    update: hook.updateSatuan,
    remove: hook.deleteSatuan,
  };

  const extraColumns = [
    {
      name: <span>Nama Satuan</span>,
      grow: 1.6,
      minWidth: '220px',
      cell: (row) => (
        <div className="py-1.5 min-w-0 flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-md bg-violet-50 text-violet-600 shrink-0">
            <Ruler className="h-3.5 w-3.5" />
          </div>
          <div className="min-w-0">
            <div className="text-sm font-semibold text-slate-800 truncate">{row.name}</div>
          </div>
        </div>
      ),
    },
    {
      name: <span>Deskripsi</span>,
      grow: 1.4,
      minWidth: '200px',
      cell: (row) => (
        <span className="text-xs text-slate-600 line-clamp-2">{row.description || '-'}</span>
      ),
    },
    {
      name: <span>Dibuat</span>,
      width: '130px',
      cell: (row) => <span className="text-xs text-slate-500">{row.created_at || '-'}</span>,
    },
  ];

  return (
    <MasterDataTablePage
      storageKey="satuan_state_v1"
      title="Master Satuan"
      subtitle="Kelola data satuan"
      accent="indigo"
      icon={Ruler}
      hook={mappedHook}
      filterFields={[
        { key: 'name', placeholder: 'Nama satuan' },
        { key: 'description', placeholder: 'Deskripsi' },
      ]}
      extraColumns={extraColumns}
      AddEditModal={AddEditSatuanModal}
      DeleteModal={DeleteConfirmationModal}
      addLabel="Tambah"
      entityLabel="Satuan"
      rowNameKey="name"
    />
  );
};

export default SatuanPage;
