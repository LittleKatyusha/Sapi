...
import ActionButton from "./persetujuanRph/components/ActionButton";

// Komponen dan hooks terpisah
import CardView from "./persetujuanRph/components/CardView";
import AddEditPersetujuanRphModal from "./persetujuanRph/modals/AddEditPersetujuanRphModal";
import PersetujuanRphDetailModal from "./persetujuanRph/modals/PersetujuanRphDetailModal";
import DeleteConfirmationModal from "./persetujuanRph/modals/DeleteConfirmationModal";
import Notification from "./persetujuanRph/components/Notification";
import usePersetujuanRph from "./persetujuanRph/hooks/usePersetujuanRph";
import customTableStyles from "./persetujuanRph/constants/tableStyles";

// Main Page
const PersetujuanRphPage = () => {
  // State
  ...
  }, [fetchPersetujuanRph, lastRefreshTime]);
  ...
  const handleSave = useCallback(async (data) => {
    try {
      if (editData) {
        await updatePersetujuanRph(editData.pid || editData.pubid, data);
        showNotification('success', 'Persetujuan RPH berhasil diperbarui');
        console.log('✅ Persetujuan RPH berhasil diupdate');
      } else {
        await createPersetujuanRph(data);
        showNotification('success', 'Persetujuan RPH berhasil ditambahkan');
        console.log('✅ Persetujuan RPH berhasil dibuat');
      }
  ...
  }, [editData, updatePersetujuanRph, createPersetujuanRph, showNotification]);
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    setIsDeleting(true);
    try {
      await deletePersetujuanRph(deleteData.pid || deleteData.pubid);
      setDeleteData(null);
      showNotification('success', 'Persetujuan RPH berhasil dihapus');
  ...
  }, [deleteData, deletePersetujuanRph, showNotification]);
  ...
  const filteredData = useMemo(() => {
    let result = persetujuanRph;
    
    // Apply client-side search filtering
    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      result = persetujuanRph.filter(item =>
    ...
    console.log('📊 PersetujuanRph data:', result.length, 'items');
    return result;
  }, [persetujuanRph, searchTerm]);
      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-1 sm:mb-2">
                Manajemen Persetujuan RPH
              </h1>
              <p className="text-gray-600 text-sm sm:text-base">
                Kelola data master persetujuan RPH
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4 md:gap-6">
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-lg">
            <h3 className="text-xs sm:text-sm font-medium opacity-90">Total Persetujuan RPH</h3>
        ...
        <div className="bg-white rounded-2xl p-3 sm:p-6 shadow-lg border border-gray-100">
          <div className="flex flex-col gap-3 sm:flex-row sm:gap-4 sm:items-center sm:justify-between">
            <div className="relative flex-1 max-w-full sm:max-w-md">
              ...
              <input
                type="text"
                placeholder="Cari persetujuan RPH..."
                value={searchTerm}
          ...
          <div>
            {viewMode === "table" ? (
              <div className="w-full min-w-[600px]">
                <DataTable
                  key={`datatable-persetujuanrph-${filteredData.length}`}
                  columns={columns}
                  data={filteredData}
                  pagination
                  ...
                  noDataComponent={
                    <div className="text-center py-12">
                      <p className="text-gray-500 text-lg">Tidak ada data persetujuan RPH ditemukan</p>
                    </div>
                  }
              ...
              <div className="p-2 sm:p-6">
                <CardView
                  key={`cardview-persetujuanrph-${filteredData.length}`}
                  data={filteredData}
                  onEdit={handleEdit}
                  onDelete={handleDelete}
                  onDetail={handleDetail}
        ...
        {deleteData && (
          <DeleteConfirmationModal
            ...
            message={`Apakah Anda yakin ingin menghapus persetujuan RPH "${deleteData?.name}"?`}
          />
        )}
...
export default PersetujuanRphPage;