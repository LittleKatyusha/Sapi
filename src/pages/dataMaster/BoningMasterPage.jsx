...
import {
  ...
  MoreVertical, Eye, Pencil, Trash2, Layers, AlertCircle,
...
const STORAGE_KEY = 'boning_state_v1';

const BoningMasterPage = () => {
  const { data: rawData, loading, error, fetchData, createItem, updateItem, deleteItem } = useBoning();
  ...
  const [filterInput, setFilterInput] = useState({ name: '', kode: '', description: '' });
  const [appliedFilters, setAppliedFilters] = useState({ name: '', kode: '', description: '' });
  ...
  const handleResetFilter = useCallback(() => {
    const empty = { name: '', kode: '', description: '' };
      ...
      <header className="shrink-0 border-b border-slate-200 bg-white">
        <div className="flex items-center justify-between gap-4 px-4 sm:px-6 py-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-bold tracking-tight text-slate-900 truncate">Master Boning</h1>
              <p className="text-xs text-slate-500 truncate hidden sm:block">Kelola data master item boning / potongan daging</p>
            </div>
          </div>
          <button
              ...
              noDataComponent={
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    {hasAppliedFilters ? 'Coba ubah filter atau reset' : 'Belum ada boning terdaftar'}
                  </p>
                </div>
              }
              highlightOnHover
    ...
    <div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right">
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Detail Boning</h3>
            <p className="text-xs text-slate-500">Informasi lengkap item boning</p>
          </div>
        ...
        <div className="shrink-0 flex gap-2 px-4 py-3 border-t border-slate-100 bg-slate-50/50">
          ...
          >
            <Pencil className="h-3.5 w-3.5" /> Edit Boning
          </button>
        </div>
      </div>
    </div>,
