...
const CardView = ({ data, onEdit, onDelete, onDetail, openMenuId, setOpenMenuId }) => (
    <div className="space-y-6">
        {data.map(item => (
            <div key={item.id} className="group bg-white border border-gray-200 rounded-3xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 relative">
                ...
                <div className="relative z-10">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center">
                            <div className="w-14 h-14 bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl flex items-center justify-center mr-4 shadow-lg group-hover:scale-110 transition-transform duration-300">
                                <Tag className="w-7 h-7 text-white" />
                            </div>
                            <div>
                                <p className="font-bold text-gray-800 font-mono text-xl mb-1">{item.id}</p>
                                <div className="flex items-center text-gray-500">
                                    <Package className="w-4 h-4 mr-1" />
                                    <span className="text-sm">{item.jenisHewan}</span>
                                </div>
                            </div>
                        </div>
                        <CardActionButton 
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            {item.tanggalPemasangan && (
                                <div className="flex items-center text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded-full">
                                    ...
                                    <span>{item.tanggalPemasangan}</span>
                                </div>
                            )}
                        </div>
                        {item.deskripsi && (
