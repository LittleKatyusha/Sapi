...
const useItemOvk = () => {
  ...
  const API_BASE = API_ENDPOINTS.MASTER.ITEM_OVK;

  const fetchItemOvk = useCallback(async (params = {}) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const createItemOvk = useCallback(async (payload) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const updateItemOvk = useCallback(async (pid, payload) => {
    setLoading(true);
    setError(null);
    try {
