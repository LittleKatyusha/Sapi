...
import ActionButton from './pembeliHo/components/ActionButton';
import customTableStyles from './pembeliHo/constants/tableStyles';
...
const PembeliHoPage = () => {
  ...
  const { data, loading, error, fetchData, createItem, updateItem, deleteItem, searchTerm, setSearchTerm, stats } = usePembeliHo();
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = data.filter((i) => i.name?.toLowerCase().includes(lower) || i.description?.toLowerCase().includes(lower));
    }
    return result.map((item, idx) => ({ order_no: idx + 1, ...item }));
  }, [data, searchTerm]);
  ...
  const columns = useMemo(() => [
    {
      ...
      width: '70px',
      cell: (row) => <span className="text-sm text-gray-500">{row.order_no}</span>,
    },
    {
      name: 'Nama',
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Pembeli HO</h1>
                <p className="text-gray-500 text-sm">Kelola data master pembeli Head Office</p>
              </div>
            </div>
            <button
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white p-5 rounded-2xl shadow-lg">
            <p className="text-sm opacity-80">Total Pembeli HO</p>
