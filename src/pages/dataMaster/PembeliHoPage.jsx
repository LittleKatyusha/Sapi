...
import {
  ...
  MoreVertical, Eye, Pencil, Trash2, Users, AlertCircle,
  ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, X,
} from 'lucide-react';

import usePembeliHo from './pembeliHo/hooks/usePembeliHo';
import AddEditPembeliHoModal from './pembeliHo/modals/AddEditPembeliHoModal';
import DeleteConfirmationModal from './pembeliHo/modals/DeleteConfirmationModal';
import Notification from './pembeliHo/components/Notification';

const STORAGE_KEY = 'pembeli_ho_state_v1';

const PembeliHoPage = () => {
  const { loading, error, fetchData, createItem, updateItem, deleteItem } = usePembeliHo();
  ...
  const handleSave = useCallback(async (payload) => {
    try {
      if (editData) {
        const result = await updateItem(editData.pid || editData.pubid, payload);
        if (result.success) {
          showNotif('success', 'Pembeli HO berhasil diperbarui');
          setShowModal(false);
          setEditData(null);
          setFetchTrigger((t) => t + 1);
        } else {
        ...
        if (result.success) {
          showNotif('success', 'Pembeli HO berhasil ditambahkan');
          setShowModal(false);
          setFetchTrigger((t) => t + 1);
        } else {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteItem(deleteData.pid || deleteData.pubid);
      if (result.success) {
        setDeleteData(null);
        showNotif('success', 'Pembeli HO berhasil dihapus');
        setFetchTrigger((t) => t + 1);
      } else {
      ...
      },
      {
        ...
        cell: (row) => (
          <div className="py-1.5 min-w-0">
            ...
            {row.description && (
              <div className="text-xs text-slate-500 truncate max-w-[320px]">{row.description}</div>
            )}
          </div>
        ),
      },
        ...
        <div className="flex items-center justify-between gap-4 px-4 sm:px-6 py-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base font-bold tracking-tight text-slate-900 truncate">Master Pembeli HO</h1>
              <p className="text-xs text-slate-500 truncate hidden sm:block">Kelola data master pembeli Head Office</p>
            </div>
          </div>
          <button
            onClick={() => { setEditData(null); setShowModal(true); }}
            className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-indigo-600 px-3.5 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 transition-colors shrink-0"
          >
          ...
          <div className="shrink-0 flex flex-col gap-3 border-b border-slate-100 px-4 py-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                Filter
                {hasAppliedFilters && (
                  <span className="inline-flex items-center justify-center rounded-full bg-indigo-600 px-1.5 text-[10px] font-bold text-white">
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  <input
                    ...
                    className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-100"
                  />
                  <input
                    type="text"
              ...
              noDataComponent={
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    {hasAppliedFilters ? 'Coba ubah filter atau reset' : 'Belum ada pembeli HO terdaftar'}
                  </p>
                </div>
              }
              highlightOnHover
          ...
          <div className="shrink-0 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-t border-slate-100 px-4 py-2.5 bg-white">
            <div className="flex items-center gap-2 text-xs text-slate-600">
              ...
              <select
                ...
                className="rounded-md border border-slate-200 bg-white px-1.5 py-1 text-xs outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-100"
              >
                {[10, 25, 50, 100].map((n) => (
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-1 px-2 text-xs text-slate-600">
                ...
                <input
                  ...
                  className="w-12 rounded-md border border-slate-200 px-1.5 py-1 text-xs text-center outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-100"
                />
    ...
    <div className="fixed inset-0 z-[9999] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right">
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Detail Pembeli HO</h3>
            <p className="text-xs text-slate-500">Informasi lengkap pembeli HO</p>
          </div>
        ...
        <div className="shrink-0 flex gap-2 px-4 py-3 border-t border-slate-100 bg-slate-50/50">
          ...
          <button
            onClick={() => onEdit(row)}
            className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white hover:bg-indigo-700 transition-colors"
          >
            <Pencil className="h-3.5 w-3.5" /> Edit Pembeli HO
          </button>
        </div>
      </div>
    </div>,
