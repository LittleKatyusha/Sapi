...
const ActionMenu = ({ item, onEdit, onDelete, onClose, buttonRef }) => {
  ...
  const actions = [
    { label: 'Edit', icon: Edit, onClick: () => onEdit(item), bg: 'bg-amber-100', text: 'text-amber-600', description: 'Ubah data daging' },
    { divider: true },
    { label: 'Hapus', icon: Trash2, onClick: () => onDelete(item), bg: 'bg-red-100', text: 'text-red-600', description: 'Hapus data daging' },
  ];
    ...
    <div ref={menuRef} style={menuStyle} className="w-48 bg-white rounded-xl shadow-2xl border border-gray-100 py-2">
      {actions.map((action, i) =>
        action.divider ? (
          ...
          <button key={i} onClick={() => { action.onClick(); onClose(); }} className="w-full text-left px-4 py-2.5 hover:bg-gray-50 transition-colors flex items-center gap-3">
