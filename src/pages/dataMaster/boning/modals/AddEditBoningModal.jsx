...
const AddEditBoningModal = ({ item, onClose, onSave }) => {
  const [formData, setFormData] = useState({ name: '', kode: '', description: '', order_no: '' });
  const [errors, setErrors] = useState({});
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    if (item) {
      setFormData({ name: item.name || '', kode: item.kode || '', description: item.description || '', order_no: item.order_no ?? '' });
    } else {
      setFormData({ name: '', kode: '', description: '', order_no: '' });
    }
    setErrors({});
  }, [item]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">{item ? 'Edit Boning' : 'Tambah Boning'}</h2>
