...
const useBoning = () => {
  ...
  const createItem = useCallback(async (payload) => {
    ...
    setError(null);
    try {
      const result = await HttpClient.post(`${API_ENDPOINTS.MASTER.BONING}/store`, payload);
      if (result?.status === 'ok' || result?.data) {
        HttpClient.clearCache?.('boning');
        await fetchData();
        return result;
      }
  ...
  const updateItem = useCallback(async (pid, payload) => {
    ...
    setError(null);
    try {
      const result = await HttpClient.post(`${API_ENDPOINTS.MASTER.BONING}/update`, { pid, ...payload });
      if (result?.status === 'ok' || result?.data) {
        HttpClient.clearCache?.('boning');
        await fetchData();
        return result;
  ...
  const deleteItem = useCallback(async (pid) => {
    ...
    setError(null);
    try {
      const result = await HttpClient.post(`${API_ENDPOINTS.MASTER.BONING}/hapus`, { pid });
