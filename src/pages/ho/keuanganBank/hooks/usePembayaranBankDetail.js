...
/**
 * Custom hook for managing payment detail data and state for Keuangan Bank
 */
export const usePembayaranBankDetail = (id, getPembayaranDetail) => {
  ...
  const createDefaultPaymentData = (id, fallbackData = null) => ({
    ...
    // Additional fields for Keuangan Bank
      ...
      if (id && !hasFetchedData.current) {
        try {
          ...
          if (isMounted && result.success) {
            ...
            console.warn('No detail data found for pembayaran bank:', id);
          ...
          // Only update state if component is still mounted
          if (isMounted) {
            console.error('Error fetching pembayaran bank detail:', err);
            setNotification({
              type: 'error',
              message: err.message || 'Gagal memuat detail pembayaran bank'
            });
