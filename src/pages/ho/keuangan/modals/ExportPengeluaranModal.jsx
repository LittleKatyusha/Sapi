...
import { X, FileSpreadsheet, FileText, Loader2, Calendar, AlertTriangle, Tag, CircleDot } from 'lucide-react';
...
const EMPTY = {
    start_date: '', end_date: '', purchase_type: '', payment_status: '',
};
...
const ExportPengeluaranModal = ({ isOpen, onClose, onConfirm, loading = false, initialFilters = {}, tipePembayaran = 1 }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setFilters({
                ...
                purchase_type: initialFilters?.purchase_type || '',
    ...
    const metodeLabel = tipePembayaran === 1 ? 'KAS' : 'BANK';
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" style={{ zIndex: 10001 }}>
                <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white rounded-t-2xl">
                    <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                        {format === 'excel' ? <FileSpreadsheet className="w-6 h-6 text-blue-600" /> : <FileText className="w-6 h-6 text-red-600" />}
                ...
                <div className="p-6 space-y-4">
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800 flex items-start gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-xs mt-1">Metode: <strong>{metodeLabel}</strong> | Range tanggal <strong>maksimal 1 bulan</strong>.</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <button
                                ...
                                className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border-2 text-sm font-medium transition-all ${format === 'excel' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}
                            >
                        ...
                        <p className="text-xs text-gray-500 mt-1.5">
                            {format === 'excel' ? 'Excel: detail per transaksi, cocok untuk analisis data.' : 'PDF: rekap ringkasan + detail, cocok untuk laporan management.'}
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="date" value={filters.start_date} onChange={(e) => handleChange('start_date', e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" disabled={loading} />
                        </div>
                        <div>
                            ...
                            <input type="date" value={filters.end_date} onChange={(e) => handleChange('end_date', e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" disabled={loading} />
                        </div>
                    </div>
                    ...
                    <div className="border-t border-gray-100 pt-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1.5"><Tag className="w-3.5 h-3.5 inline mr-1" />Jenis Pembelian</label>
                                <SearchableSelect options={JENIS_OPTIONS} value={filters.purchase_type} onChange={(v) => handleChange('purchase_type', v || '')} placeholder="Semua Jenis" disabled={loading} menuZIndex={10050} />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1.5"><CircleDot className="w-3.5 h-3.5 inline mr-1" />Status Pembayaran</label>
                                <SearchableSelect options={STATUS_OPTIONS} value={filters.payment_status} onChange={(v) => handleChange('payment_status', v || '')} placeholder="Semua Status" disabled={loading} menuZIndex={10050} />
                            </div>
                        </div>
                    </div>
...
export default ExportPengeluaranModal;
