...
import {
  Search, X, Loader2, Package, RefreshCw, TrendingUp, Layers, DollarSign, BarChart2
} from 'lucide-react';
...
const StockBadge = ({ jumlah }) => {
  ...
  return <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">Tersedia</span>;
};

// ─── Page Component ────────────────────────────────────────────────────────────

const StokFeedmilHoPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [dataRes, summaryRes] = await Promise.all([
        StokFeedmilHoService.getData(),
        StokFeedmilHoService.getSummary(),
      ]);
      const rows = dataRes?.data ?? dataRes ?? [];
      setAllData(Array.isArray(rows) ? rows : []);
      setSummary(summaryRes?.data ?? summaryRes ?? { total: 0 });
    } catch (err) {
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Monitoring sisa stok bahan pakan Head Office</p>
        </div>
        <button
          onClick={fetchData}
          disabled={loading}
          className="flex items-center gap-2 px-3 py-2 rounded-lg border border-green-200 bg-white text-green-700 hover:bg-green-50 text-sm font-medium transition disabled:opacity-50"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-300 focus:border-green-400 outline-none"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
        ...
        <DataTable
          ...
          progressComponent={
            <div className="py-10 flex items-center gap-2 text-gray-400">
              ...
              Memuat data stok feedmil...
            </div>
          }
          noDataComponent={
            <div className="py-10 text-gray-400 text-sm">
              {search ? 'Tidak ada item yang cocok dengan pencarian.' : 'Belum ada data stok feedmil.'}
            </div>
          }
...
export default StokFeedmilHoPage;
