...
/**
 * Custom hook for managing payment info card data for Keuangan Kas
 * Fetches data from /api/ho/pengeluaran/show endpoint
 */
export const usePembayaranKasInfo = (pid, getInfoCard) => {
      ...
      if (pid && !hasFetchedData.current) {
        ...
        try {
          ...
          if (isMounted && result.success) {
            ...
            console.warn('No info data found for pembayaran kas:', pid);
          ...
          // Only update state if component is still mounted
          if (isMounted) {
            console.error('Error fetching pembayaran kas info:', err);
            setError(err.message || 'Gagal memuat info pembayaran kas');
            setInfoData(null);
            setLoading(false);
          }
        }
