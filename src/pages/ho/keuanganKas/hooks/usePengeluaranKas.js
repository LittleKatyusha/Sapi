...
/**
 * Custom hook untuk mengelola data Pengeluaran Pengajuan Biaya Kas
 * Hook ini untuk approval/disbursement page - mengelola persetujuan dan penolakan pengajuan
 * Menggunakan API real dari pengeluaranPengajuanBiayaKasService
 */
const usePengeluaranKas = () => {
    ...
    const fetchPengeluaran = useCallback(async (
        ...
        try {
            ...
            const response = await pengeluaranPengajuanBiayaKasService.getData({
            ...
            const transformedData = (response.data || []).map(item => 
                pengeluaranPengajuanBiayaKasService.transformData(item)
            );
    ...
    const fetchCardData = useCallback(async () => {
        ...
        try {
            const response = await pengeluaranPengajuanBiayaKasService.getCardData();
    ...
    const approvePengajuan = useCallback(async (pid, data) => {
        ...
        try {
            ...
            const validation = pengeluaranPengajuanBiayaKasService.validateApprovalData(data);
            if (!validation.valid) {
                throw new Error(validation.errors.join(', '));
            }

            // Transform data to backend format
            const backendData = pengeluaranPengajuanBiayaKasService.transformApprovalToBackend(data);

            // Call approve service
            const response = await pengeluaranPengajuanBiayaKasService.approve(pid, backendData);
    ...
    const rejectPengajuan = useCallback(async (pid, reason) => {
        ...
        try {
            ...
            const validation = pengeluaranPengajuanBiayaKasService.validateRejectionData(reason);
            if (!validation.valid) {
                throw new Error(validation.errors.join(', '));
            }

            // Call reject service
            const response = await pengeluaranPengajuanBiayaKasService.reject(pid, reason);
    ...
    const deletePengeluaran = useCallback(async (pid) => {
        ...
        try {
            const response = await pengeluaranPengajuanBiayaKasService.delete(pid);
    ...
    const getPengeluaranDetail = useCallback(async (pid) => {
        ...
        try {
            const response = await pengeluaranPengajuanBiayaKasService.getDetail(pid);
    ...
    return {
        ...
        getStatusInfo: pengeluaranPengajuanBiayaKasService.getStatusInfo,
        formatCurrency: pengeluaranPengajuanBiayaKasService.formatCurrency,
        formatDate: pengeluaranPengajuanBiayaKasService.formatDate,
        getFileUrl: pengeluaranPengajuanBiayaKasService.getFileUrl
    };
};
