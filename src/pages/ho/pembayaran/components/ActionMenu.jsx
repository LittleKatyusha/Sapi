...
const ActionMenu = ({ row, onEdit, onDelete, onDetail, onClose, buttonRef }) => {
    ...
    const handleViewFile = async (row) => {
        if (!row.file) {
            alert('File tidak tersedia untuk pembayaran ini');
            return;
        }
    ...
    const handleDownloadReport = async (row) => {
        ...
        try {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `Laporan_Pembayaran_${row.nota || 'Report'}.pdf`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
                        ...
                        <button
                            ...
                            onClick={() => { 
                                if (!action.disabled) {
                                    ...
                                    if (action.label === 'Detail Pembayaran' || action.label === 'Edit Pembayaran' || action.label === 'Hapus Pembayaran') {
                                        onClose(); 
                                    }
                                }
                            }}
