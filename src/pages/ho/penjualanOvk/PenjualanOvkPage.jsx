...
import penjualanOvkService from '../../../services/penjualanOvkService';
...
const PenjualanOvkPage = () => {
  useDocumentTitle('Penjualan OVK HO');
  ...
  const fetchData = useCallback(async () => {
    ...
    const res = await penjualanOvkService.getData(params);
    setLoading(false);
    if (res.success) {
  ...
  const handleDetail = async (row) => {
    ...
    setDetailData(null);
    const res = await penjualanOvkService.show(row.pid);
  ...
  const handleHistory = async (row) => {
    ...
    const res = await penjualanOvkService.getPaymentHistory(row.pid);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8">
      <div className="w-full max-w-full mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Penjualan OVK HO</h1>
          <p className="text-sm text-gray-500 mt-1">Histori penjualan OVK HO ke RPH (read-only — transaksi dipicu dari sisi RPH)</p>
        </div>
          ...
          <DataTable
            ...
            noDataComponent={
              <div className="py-12 text-center text-gray-400">
                ...
                <p className="text-sm">Belum ada penjualan OVK</p>
              </div>
            }
            customStyles={{
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-gray-200">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Detail Penjualan OVK</h2>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="text-right px-3 py-2 text-xs font-semibold text-gray-600">Harga Jual</th>
                        <th className="text-right px-3 py-2 text-xs font-semibold text-gray-600">HPP/unit</th>
                      ...
                      {(detailData.details || []).map((d, i) => (
                        <tr key={i}>
                          <td className="px-3 py-2 text-gray-800">
                            ...
                            <div className="font-mono text-xs text-gray-500">{d.nama_satuan}</div>
                          </td>
                          ...
                          <td className="px-3 py-2 text-right text-gray-600">{formatRupiah(d.hpp_per_unit)}</td>
...
export default PenjualanOvkPage;
