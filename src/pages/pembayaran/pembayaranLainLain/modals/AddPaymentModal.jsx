...
const AddPaymentModal = ({ isOpen, onClose, onSuccess, pembayaranId, pembayaranData }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            console.log('AddPaymentModal opened with:', {
                pembayaranId,
                pembayaranData,
                hasId: !!pembayaranId
            });
        }
    }, [isOpen, pembayaranId, pembayaranData]);
    ...
    const handleSubmit = async (e) => {
        ...
        // Check if pembayaranId is available
        if (!pembayaranId) {
            setNotification({
                type: 'error',
                message: 'ID Pembayaran tidak ditemukan. Silakan refresh halaman.'
            });
            return;
        }
