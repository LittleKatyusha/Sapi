...
const PembayaranDetailPage = () => {
  ...
  const getDetail = useCallback(async (idToFetch) => {
      if (isPurchase) {
          console.log('🔄 Feedmill Fetcher: Fetching as Purchase (Keuangan Context)', idToFetch);
          try {
              ...
              if (response && (response.success || response.data)) {
                  ...
                  const header = {
                      ...purchaseData,
                      id: purchaseData.id || purchaseData.pid,
                      encryptedPid: purchaseData.pid || idToFetch,
                      id_pembelian: purchaseData.id || purchaseData.pid,
  ...
  const handleEdit = () => navigate(`/pembayaran/feedmill/edit/${id}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => navigate('/pembayaran/feedmill'), 1500);
      } else {
