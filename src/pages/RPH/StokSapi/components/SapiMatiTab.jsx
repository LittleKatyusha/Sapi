...
const SapiMatiTab = ({ onRefresh }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await StokSapiService.getSapiMatiData({
        start: 0,
        length: 100,
        _t: Date.now(),
      });
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-800">Riwayat Sapi Mati</h3>
        <button
          onClick={handleRefresh}
          disabled={loading}
          className="inline-flex items-center gap-1.5 rounded-md bg-slate-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
              ...
              <tr>
                <td colSpan={8} className="px-3 py-8 text-center">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <p className="text-sm font-medium text-slate-700 mb-1">Belum Ada Data Sapi Mati</p>
                    <p className="text-xs text-slate-500">Data sapi mati akan muncul di sini setelah dicatat.</p>
                  </div>
                </td>
              </tr>
            ) : (
