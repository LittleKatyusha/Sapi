...
import { X, Wallet, Loader2, History, ChevronLeft, ChevronRight, Search, AlertCircle } from 'lucide-react';
...
const BayarHutangModal = ({ isOpen, onClose, pedagangData, onSubmit }) => {
  ...
  const fetchHistory = useCallback(async (p, searchTerm = '') => {
    ...
    try {
      drawRef.current += 1;
      const result = await PedagangService.getHutangHistory({
  ...
  const currentHutang = Number(pedagangData?.saldo_beku ?? 0);
  const nominalNum = parseRupiah(nominal);
  const newHutang = nominalNum === '' ? currentHutang : Math.max(0, currentHutang - Number(nominalNum));
  ...
  const handleSubmit = async (e) => {
    ...
    if (Number(nominalNum) > currentHutang) {
      setError(`Nominal melebihi sisa hutang (Rp ${new Intl.NumberFormat('id-ID').format(currentHutang)})`);
      return;
    }
    setSubmitting(true);
    setError('');
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
      <div className="bg-white rounded-2xl w-full max-w-4xl flex flex-col overflow-hidden shadow-2xl max-h-[92vh]">
        {/* Header */}
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-rose-50 to-white">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-rose-600 flex items-center justify-center text-white">
              <Wallet className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-base font-bold text-gray-800">Bayar Hutang</h3>
              <p className="text-xs text-gray-500">Bayar hutang pedagang (kurangi saldo beku)</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-xl p-3">
              ...
              <div className="mt-2 flex items-center justify-between text-xs">
                <span className="text-gray-500">Hutang saat ini</span>
                <span className="font-semibold text-rose-700 tabular-nums">{formatCurrency(currentHutang)}</span>
              </div>
            </div>
            <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 flex flex-col justify-center">
              <p className="text-[11px] text-rose-600 uppercase font-semibold">Sisa Hutang setelah bayar</p>
              <p className="text-lg font-bold text-rose-800 tabular-nums mt-1">{formatCurrency(newHutang)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2">
              <label className="block text-[11px] font-semibold text-gray-600 uppercase tracking-wide mb-1.5" htmlFor="hutang-nominal">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0"
                  autoFocus
                  className={`w-full pl-9 pr-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition ${
            ...
            <div>
              <label className="block text-[11px] font-semibold text-gray-600 uppercase tracking-wide mb-1.5" htmlFor="hutang-note">
                Catatan
              </label>
              <input
                id="hutang-note"
                ...
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition"
                disabled={submitting}
              />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={submitting || currentHutang <= 0}
              className="px-6 py-2.5 text-sm font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-lg transition disabled:opacity-50 flex items-center gap-2"
            >
              {submitting ? (
                <>
          ...
          <div className="border-t border-gray-100 pt-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-bold text-gray-800">History Pembayaran Hutang</h4>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-7 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition w-56"
              ...
              <table className="w-full text-xs min-w-[640px]">
                <thead className="bg-gray-50 text-gray-500 uppercase text-[10px] tracking-wide">
                  <tr>
                    ...
                    <th className="px-4 py-2.5 text-right font-semibold">Sisa Hutang</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {historyLoading ? (
                    ...
                    <tr>
                      <td colSpan={6} className="px-4 py-10 text-center text-gray-400">
                        Belum ada history pembayaran hutang
                      </td>
                    </tr>
                  ) : (
                    history.map((row, idx) => (
                      <tr key={row.pid || idx} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-4 py-2.5">
                          <span className="inline-flex px-2 py-0.5 rounded-full bg-rose-50 text-rose-700 text-[10px] font-semibold">
                            {row.jenis_transaksi || '-'}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-2.5 text-right text-gray-700 tabular-nums whitespace-nowrap">
                          {formatCurrency(row.nominal_hutang)}
                        </td>
...
export default BayarHutangModal;
