...
const usePenjualanKarkas = () => {
  ...
  const fetchData = useCallback(async (
    page = 1, perPage = 10, search = '', startDate = '', endDate = ''
  ) => {
    setLoading(true);
    setError(null);
    drawRef.current += 1;
    try {
      const res = await PenjualanKarkasService.getData({
  ...
  // Karkas has no dedicated getPedagang endpoint; reuse Boning's
  ...
  const fetchHarga = useCallback(async (pidPedagang) => {
    return PenjualanKarkasService.getHarga(pidPedagang);
  }, []);

  const show = useCallback(async (pid) => {
    return PenjualanKarkasService.show(pid);
  }, []);

  const store = useCallback(async (payload) => {
    setCreateLoading(true);
    try {
      return await PenjualanKarkasService.store(payload);
    } finally {
  ...
  const update = useCallback(async (payload) => {
    ...
    try {
      return await PenjualanKarkasService.update(payload);
    } finally {
  ...
  const hapus = useCallback(async (pid) => {
    ...
    try {
      return await PenjualanKarkasService.hapus(pid);
    } finally {
...
export default usePenjualanKarkas;
