...
import service from '../../../../../services/penjualanKarkasService';

export default function useKarkasDocument() {
  ...
  const download = async (type, row) => {
    ...
    try {
      ...
      try {
        link.href = url;
        link.download = `${type === 'nota' ? 'Nota_Karkas' : 'Surat_Jalan_Karkas'}_${String(row.no_kwitansi || 'transaksi').replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 100)}.pdf`;
