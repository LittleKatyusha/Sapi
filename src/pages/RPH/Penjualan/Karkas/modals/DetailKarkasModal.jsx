...
const getBagianLabel = (value) => {
  const found = BAGIAN_KARKAS.find(b => b.value === value);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 my-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-800">Detail Penjualan Karkas</h2>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-medium text-gray-600">Bagian Karkas</th>
                      <th className="px-3 py-2 text-center font-medium text-gray-600">Berat Bersih</th>
                      <th className="px-3 py-2 text-center font-medium text-gray-600">Berat Kulit</th>
                  ...
                  <tbody>
                    {(detail || []).map((item, idx) => (
                      <tr key={idx} className="border-t border-gray-100">
                        ...
                        <td className="px-3 py-2 font-medium text-gray-800">{getBagianLabel(item.bagian_karkas)}</td>
                        <td className="px-3 py-2 text-center">{item.berat_bersih ?? '-'} kg</td>
                        <td className="px-3 py-2 text-center">{item.berat_kulit ? `${item.berat_kulit} kg` : '-'}</td>
