...
/**
 * Modal server-side datatable untuk memilih induk (betina/jantan) dari stok DOKA tersedia.
 *
 * Props:
 * - open: boolean
 * - onClose: () => void
 ...
 * - title: string
 */
const ParentPickerDokaModal = ({ open, onClose, onSelect, jenisKelamin, title }) => {
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const [searchQuery, setSearchQuery] = useState('');
  const [klasifikasiFilter, setKlasifikasiFilter] = useState('');
  const [showAdvancedFilter, setShowAdvancedFilter] = useState(false);
  const [klasifikasiOptions, setKlasifikasiOptions] = useState([]);

  const searchRef = useRef('');
  const klasifikasiRef = useRef('');
  ...
  const hasActiveFilter = searchQuery || klasifikasiFilter;

  const fetchData = useCallback(async (opts = {}) => {
    const {
      page = pageRef.current,
      q = searchRef.current,
      jenisKlasifikasi = klasifikasiRef.current,
    } = opts;

    setLoading(true);
    setError(null);
    try {
      const res = await StokDokaService.parentOptions(jenisKelamin, {
        q,
        jenisKlasifikasi,
  ...
  useEffect(() => {
    ...
    searchRef.current = '';
    klasifikasiRef.current = '';
    pageRef.current = 1;
    setCurrentPage(1);
    fetchData({ page: 1, q: '', jenisKlasifikasi: '' });
  ...
  const handleSearch = () => {
    ...
    klasifikasiRef.current = klasifikasiFilter;
    pageRef.current = 1;
    setCurrentPage(1);
    fetchData({ page: 1 });
  };
      ...
      <div className="flex max-h-[90vh] w-full max-w-5xl flex-col rounded-xl bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <div>
            ...
            <p className="text-xs text-gray-500">Pilih dari stok DOKA tersedia di RPH</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            ...
            <div className="flex flex-1 flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Eartag, klasifikasi, no PO..."
          ...
          {showAdvancedFilter && (
            <div className="mt-3 grid grid-cols-1 gap-3 rounded-lg bg-gray-50/50 p-3 sm:grid-cols-2">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-gray-500">Klasifikasi (Kambing/Domba)</label>
                <select
              ...
              <div className="flex items-end">
                {hasActiveFilter ? (
                  <button type="button" onClick={handleReset} className="text-xs text-emerald-600 font-medium hover:underline">
                    {recordsFiltered} dari {recordsTotal} DOKA · Reset filter
                  </button>
                ) : (
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white">
                ...
                <th className="py-2 px-3 text-left font-semibold border border-emerald-500 whitespace-nowrap">Jenis Hewan</th>
            ...
            <tbody>
              ...
              {!loading && rows.map((row, index) => (
                ...
                >
                  ...
                  <td className="py-2 px-3 border border-gray-100 whitespace-nowrap text-gray-700">{row.jenis_hewan || '-'}</td>
                  <td className="py-2 px-3 border border-gray-100 whitespace-nowrap text-gray-700">{row.jenis_klasifikasi || '-'}</td>
        ...
        <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3">
          <span className="text-xs text-gray-500">
            {recordsFiltered > 0
              ? `Menampilkan ${(currentPage - 1) * perPage + 1}-${Math.min(currentPage * perPage, recordsFiltered)} dari ${recordsFiltered} DOKA`
              : 'Tidak ada data'}
          </span>
...
export default ParentPickerDokaModal;
