...
/**
 * Hook untuk managing Return Penjualan Doka Utuh operations
 */
const useReturnPenjualan = () => {
    ...
    /**
     * Create return for penjualan doka utuh (supports multiple cattle)
     */
    const createReturn = useCallback(async (data) => {
        ...
        try {
            ...
            const response = await HttpClient.post(
                `${API_ENDPOINTS.RPH.RETURN_PENJUALAN_DOKA_UTUH}`,
                payload
            );
    ...
    const getReturnHistory = useCallback(async (idPenjualan) => {
        ...
        try {
            const response = await HttpClient.get(
                `${API_ENDPOINTS.RPH.RETURN_PENJUALAN_DOKA_UTUH}?id_penjualan=${idPenjualan}`
            );
