...
const defaultFormData = {
  ...
  tanggal_transaksi: new Date().toISOString().split('T')[0], tipe_penjualan: 'tunai', jangka_waktu: '', jenis_transaksi: 'doka_utuh',
  biaya_kirim: '',
...
const DokaPickerModal = ({ isOpen, onClose, onSelect, availableDoka, excludeIds = [], currentValue }) => {
  const [search, setSearch] = useState('');
  ...
  const filtered = useMemo(() => {
    ...
    const list = availableDoka.filter((s) => !excludedSet.has(String(s.doka_id)));
    if (!q) return list;
    return list.filter((s) =>
      ...
      String(s.jenis_doka || '').toLowerCase().includes(q)
    );
  }, [availableDoka, excludedSet, search]);
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col">
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-800">Pilih Doka</h3>
              <p className="text-xs text-gray-500">{filtered.length} doka tersedia</p>
            </div>
          </div>
        ...
        <div className="px-5 py-3 border-b border-gray-100 bg-gray-50/50">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Cari eartag, code eartag, atau jenis doka..."
        ...
        <div className="flex-1 overflow-auto">
          {filtered.length === 0 ? (
            <div className="p-12 flex flex-col items-center justify-center text-gray-400">
              ...
              <p className="text-sm font-medium">Tidak ada doka tersedia</p>
              <p className="text-xs mt-1">Coba kata kunci lain atau doka sudah dipilih semua</p>
            </div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((s) => {
                  const isSelected = String(currentValue) === String(s.doka_id);
                  return (
                    <tr
                      key={s.doka_id}
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-gray-700">{s.jenis_doka || '-'}</td>
...
const AddPenjualanDokaUtuhPageV2 = () => {
  const navigate = useNavigate();
  const { pid } = useParams();
  const isEdit = !!pid;

  const { loading, create, update, fetchDetail, fetchAvailableDoka } = usePenjualanDokaUtuh();
  const [notification, setNotification] = useState({ isVisible: false, type: 'info', message: '' });
  const [errors, setErrors] = useState({});
  const [formData, setFormData] = useState({ ...defaultFormData });
  const [availableDoka, setAvailableDoka] = useState([]);
  ...
  useEffect(() => {
    const loadDoka = async () => {
      // eslint-disable-next-line no-console
      console.log('Loading doka for jenis_transaksi:', formData.jenis_transaksi);
      const result = await fetchAvailableDoka(formData.jenis_transaksi);
      // eslint-disable-next-line no-console
      console.log('Available doka result:', result);
      if (result.success) setAvailableDoka(result.data || []);
    };
    loadDoka();
  }, [fetchAvailableDoka, formData.jenis_transaksi]);
  ...
  const loadEditData = useCallback(async () => {
    ...
    if (result.success && result.data) {
      ...
      setFormData({
        ...
        tipe_penjualan: d.tipe_penjualan || 'tunai', jangka_waktu: d.jangka_waktu || '', jenis_transaksi: d.jenis_transaksi || 'doka_utuh',
        ...
        details: d.details?.map((item) => ({
          doka_id: item.doka_id || '', no_eartag: item.no_eartag || '', merk: item.merk || '',
          berat: item.berat || '', harga_beli: item.harga_beli || '', harga_jual: item.harga_jual || item.subtotal || '',
          keterangan: item.keterangan || '',
        })) || [],
      });
      // Merge current transaction's doka into availableDoka so they show in dropdown during edit
      setAvailableDoka((prev) => {
        const existingIds = new Set(prev.map((s) => String(s.doka_id)));
        const transactionDoka = d.details?.filter((item) => item.doka_id && !existingIds.has(String(item.doka_id))).map((item) => ({
          doka_id: item.doka_id,
          no_eartag: item.no_eartag || '',
          jenis_doka: item.merk || 'Doka',
          berat: item.berat || '',
          harga_beli: '',
        })) || [];
        return [...prev, ...transactionDoka];
      });
  ...
  const handleAddItem = () => {
    setFormData((prev) => ({
      ...prev,
      details: [...prev.details, { doka_id: '', no_eartag: '', code_eartag: '', eartag_supplier: '', merk: '', berat: '', harga_beli: '', harga_jual: '', keterangan: '' }],
    }));
  };
  ...
  const handleItemChange = (index, field, value) => {
    if (field === 'doka_id' && value) {
      const isDuplicate = formData.details.some((it, idx) => idx !== index && String(it.doka_id) === String(value));
      if (isDuplicate) {
        showNotif('error', 'Doka dengan eartag ini sudah dipilih di baris lain. Pilih doka yang berbeda.');
        setFormData((prev) => {
          const newDetails = [...prev.details];
          newDetails[index] = { ...newDetails[index], doka_id: '', no_eartag: '', berat: '', harga_beli: '' };
    ...
    setFormData((prev) => {
      ...
      if (field === 'doka_id') {
        const selectedDoka = availableDoka.find((s) => String(s.doka_id) === String(value));
        if (selectedDoka) {
          newDetails[index].no_eartag = selectedDoka.no_eartag || '';
          newDetails[index].code_eartag = selectedDoka.code_eartag || '';
          newDetails[index].eartag_supplier = selectedDoka.eartag_supplier || '';
          newDetails[index].berat = selectedDoka.berat || '';
          newDetails[index].harga_beli = selectedDoka.harga_beli || '';
        }
      }
      return { ...prev, details: newDetails };
    });
  ...
  const handlePickDoka = (doka) => {
    const index = pickerModal.index;
    if (index == null) return;
    handleItemChange(index, 'doka_id', doka.doka_id);
  ...
  const validate = () => {
    ...
    if (formData.details.length === 0) newErrors.details = 'Minimal harus ada 1 item doka';

    // Cek duplikat doka_id dalam details
    const seenDokaIds = new Set();
    formData.details.forEach((item, index) => {
      if (!item.doka_id) {
        newErrors[`detail_${index}_doka_id`] = 'Doka wajib dipilih';
      } else if (seenDokaIds.has(String(item.doka_id))) {
        newErrors[`detail_${index}_doka_id`] = 'Doka ini sudah dipilih di baris lain';
      } else {
        seenDokaIds.add(String(item.doka_id));
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      setTimeout(() => navigate('/rph/penjualan-doka-utuh'), 1500);
    } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-emerald-100 p-4 md:p-6">
      <div className="max-w-full mx-4 md:mx-8 space-y-6">
        <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/rph/penjualan-doka-utuh')} className="p-2 hover:bg-gray-100 rounded-lg transition"><ArrowLeft className="w-5 h-5" /></button>
            <div className="flex items-center gap-3">
              <div className="p-3 bg-green-100 rounded-xl"><ShoppingCart className="h-7 w-7 text-green-600" /></div>
              <div>
                <h1 className="text-2xl font-bold text-gray-800">{isEdit ? 'Edit Penjualan Doka Utuh' : 'Tambah Penjualan Doka Utuh'}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white rounded-2xl shadow border border-gray-100">
            <div className="flex overflow-x-auto border-b border-gray-200">
              {[
                ...
                { id: 3, label: 'Detail Doka', icon: Beef, color: 'red' },
            ...
            <div className="p-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <SearchableSelect
                      ...
                      options={[
                        { value: 'doka_utuh', label: 'Doka Utuh' },
                      ]}
                      onSelect={(val) => {
              ...
              {activeTab === 3 && (
                <div>
                  <h3 className="text-lg font-bold text-red-800 mb-4 flex items-center gap-2"><Beef className="w-5 h-5" />Detail Doka</h3>
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-sm text-gray-500">Pilih doka yang akan dijual</p>
                    <button type="button" onClick={handleAddItem} className="flex items-center gap-2 bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 text-sm font-medium transition">
                      <PlusCircle className="w-4 h-4" />Tambah Doka
                    </button>
                  </div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div key={index} className="p-4 border border-gray-200 rounded-xl bg-gray-50 relative">
                        ...
                        <div className="flex flex-col xl:flex-row gap-3 items-end">
                          <div className="w-full xl:w-72">
                            <label className="block text-xs font-medium text-gray-700 mb-1">Pilih Doka <span className="text-red-500">*</span></label>
                            <button
                              type="button"
                              onClick={() => setPickerModal({ open: true, index })}
                              className={`w-full px-3 py-2.5 border rounded-lg text-left text-sm flex items-center justify-between transition ${errors[`detail_${index}_doka_id`] ? 'border-red-500 ring-1 ring-red-200' : 'border-gray-300 hover:border-emerald-400'} bg-white hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500`}
                            >
                              <span className={`truncate ${item.doka_id ? 'text-gray-900' : 'text-gray-400'}`}>
                                {item.doka_id
                            ...
                            {errors[`detail_${index}_doka_id`] && <p className="text-red-500 text-xs mt-1">{errors[`detail_${index}_doka_id`]}</p>}
                          </div>
                          ...
                          <div className="w-full xl:w-40">
                            ...
                            <input type="text" value={item.merk} onChange={(e) => handleItemChange(index, 'merk', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Merk doka" />
                          </div>
                    ...
                    {formData.details.length === 0 && (
                      <div className="text-center py-8 text-gray-400"><Beef className="mx-auto h-12 w-12 mb-3 opacity-40" /><p>Belum ada item doka. Klik "Tambah Doka" untuk menambahkan.</p></div>
                    )}
                  </div>
                </div>
              )}
          ...
          <div className="bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-2xl p-6 shadow-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div><p className="text-sm opacity-80">Total Harga Doka</p><p className="text-2xl font-bold">Rp {totals.totalHarga.toLocaleString('id-ID')}</p></div>
          ...
          <div className="flex gap-3 justify-end">
            <button type="button" onClick={() => navigate('/rph/penjualan-doka-utuh')} className="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium">Batal</button>
      ...
      <DokaPickerModal
        ...
        onSelect={handlePickDoka}
        availableDoka={availableDoka}
        excludeIds={formData.details.map((d) => d.doka_id).filter(Boolean)}
        currentValue={pickerModal.index != null ? formData.details[pickerModal.index]?.doka_id : undefined}
      />
...
export default AddPenjualanDokaUtuhPageV2;
