...
/**
 * Page untuk Return Penjualan Doka Utuh
...
const ReturnPenjualanPage = () => {
    ...
    const fetchPenjualanDetail = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`${API_BASE_URL}/api/rph/penjualan-doka-utuh/show`, {
                method: 'POST',
                headers: {
    ...
    const validateForm = () => {
        ...
        if (formData.selected_cattle.length === 0) {
            newErrors.selected_cattle = 'Minimal harus memilih 1 doka untuk di-return';
        }
        setErrors(newErrors);
    ...
    const handleSubmit = async (e) => {
        ...
        if (result) {
            ...
            setTimeout(() => {
                navigate('/rph/penjualan-doka-utuh');
            }, 1500);
        } else {
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                                    ...
                                    Pilih Doka untuk Return
                                </h2>
                                <p className="text-xs text-slate-500 mt-0.5">
                                    Centang doka & isi kondisi/catatan secara inline
                                </p>
                            </div>
                            {totalCattle > 0 && (
                                <button
                            ...
                            {cattleList.length === 0 ? (
                                <div className="bg-slate-50 rounded-xl p-12 text-center border border-dashed border-slate-300">
                                    ...
                                    <p className="text-slate-500 text-sm font-medium">
                                        {returnedCattle.length > 0
                                            ? 'Semua doka dalam transaksi ini sudah direturn'
                                            : 'Tidak ada doka dalam transaksi ini'}
                                    </p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                            ...
                            <div className="p-5 space-y-4">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-slate-500">Doka Dipilih</span>
                                ...
                                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
                                    ...
                                    <p className="text-xs text-amber-700 leading-relaxed">
                                        Grand total & sisa pembayaran akan otomatis berkurang sesuai doka yang direreturn.
                                    </p>
                                </div>
                            </div>
                        </div>
