...
const PembelianKonsentratPage = () => {
  ...
  const handleDownload = async (row, type = 'purchase') => {
    ...
    const label = type === 'stock' ? 'Rincian Batch Stok Konsentrat' : 'Rincian Pembelian Konsentrat';
    try {
      const blob = await pembelianKonsentratService.downloadDocument(row.pid, type);
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      try {
        link.href = url;
        link.download = `${label.replace(/ /g, '_')}_${String(row.nomor_faktur || row.resep_kode || 'RPH').replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 100)}.pdf`;
      ...
      cell: (row) => (
        <div className="relative">
          ...
          {openMenuId === row.pid && createPortal(
            <div
              className="fixed z-50 w-56 max-h-[calc(100vh-16px)] overflow-y-auto bg-white border border-gray-200 rounded-lg shadow-lg py-1"
              role="group"
              aria-label="Aksi pembelian konsentrat"
            ...
            >
              ...
              >
                ...
                {downloading === `purchase:${row.pid}` ? 'Mengunduh PDF...' : 'Rincian Pembelian Konsentrat PDF'}
              </button>
    ...
    },
    {
      ...
      cell: (row) => (
        <button
          ...
          aria-label={`Unduh rincian batch stok ${row.resep_kode} PDF`}
          className="px-3 py-2 text-xs text-blue-700 hover:bg-blue-50 rounded-md disabled:opacity-50"
        >
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8">
      <div className="w-full max-w-full mx-auto space-y-6">
        ...
        <span role="status" className="sr-only">{downloading ? 'Mengunduh dokumen konsentrat PDF...' : ''}</span>
